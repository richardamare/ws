...
- **AWS Bedrock** (Seyfor tenant) — internal Seyfor work where Anthropic API is not procured
...
- A referenced starter repo (e.g. `seyfor/ma.starter.web-app`)

Inherited decisions appear in your SOLUTION.md as:

> **Inherited from `seyfor/ma.starter.web-app`:** .NET 10 + ASP.NET Core Minimal
...
- "interní pro Seyfor" + AI → AWS Bedrock
...
- Audience: internal Seyfor / client-internal / customer-facing? (drives auth choice, AI tenant)
- LLM tenant: AWS Bedrock (Seyfor) / Azure OpenAI (client) / Anthropic API direct?
...
- **EU AI Act position:** classify (high-risk / limited-risk / out-of-scope) with reasoning. Document in the Seyfor AI register.
