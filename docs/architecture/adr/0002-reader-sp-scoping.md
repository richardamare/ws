...
- **Status:** Accepted
- **Date:** 2026-06-27
- **Deciders:** Richard Amare

## Context

Richard holds near-Owner (Contributor + User Access Administrator + RBAC Administrator), permanent and
...
single resource group** — one SP per project (`sp-ramare-<proj>-reader`) — isolated from the personal
...
- Easier: default sessions are read-only on one RG; `az group delete` elsewhere → AuthorizationFailed.
  Richard keeps all his roles, untouched.
