...
Richard has **near-Owner** on a **shared lab subscription**: Contributor + User Access Administrator +
...
- Naming: `sp-ramare-<project>-reader` (the `sp-ramare` prefix makes all of them filterable in Entra).

```bash
az ad sp create-for-rbac \
  --name sp-ramare-proj1-reader \
  --role Reader \
...
clearly-marked elevated tab that runs `az login` as Richard (`unset AZURE_CONFIG_DIR` → personal
...
- **PIM-eligible** would work but Richard fears losing standing access and the slow re-grant. The
...
1. Never modify Richard's existing Azure role assignments.
